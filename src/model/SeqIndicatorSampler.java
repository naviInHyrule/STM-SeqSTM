 ...
 */
public class SeqIndicatorSampler extends TopicSampler implements Serializable{
	//Serialization
	...
	public SeqIndicatorSampler(Corpus corpus,
	...
	public void initCachedValues() {
		System.out.println("Initialize cached values: "+StirNum.maxM()+"--"+StirNum.maxN());
	...
	protected int sample(final int i, final int j, final int w, int k){
		...
		//System.out.println(stables.TIJK[i][j][k]);
		//System.out.println(stables.NIJK[i][j][k]);
	...
	private double probSeqBase(final int i,  final int k)
	{
		double val = 0.0;
		if( i>0) {
			val = (modelParams.getAlphaSum()*probBase(i - 1, k) + stables.TIK[i][k])
